...
public class OrExpression implements BooleanExpression{
    ...
    public static final String KEY_WORD = "or";
    ...
    public static OrExpression parse(String string) throws ParseSyntaxException {
        String[] parts = ExpressionFactory.split(string).toArray(new String[0]);
        final int requiredLength = 2;

        if (parts.length != requiredLength) {
            throw new ParseSyntaxException("Invalid Syntax (OrExpression): expected " + requiredLength +
        ...
        if (!(leftResult instanceof BooleanExpression)) {
            ...
            return new OrExpression((BooleanExpression) leftResult, (BooleanExpression) rightResult);
        }
    }
